					...
					if (response.resultCode != 0) {
						...
						for (let i in action.venueList) {
							...
							for (let i in venue.actionEventList) {
								...
								if (typeof venue.events[m] === 'undefined') venue.events[m] = {title: S.translate.months[date[1]], days: {}};
								if (typeof venue.events[m].days[d] === 'undefined') venue.events[m].days[d] = {title: +d + ' ' + S.translate.days[+day.getDay()], times: {}};
								if (typeof venue.events[m].days[d].times[event.time] === 'undefined') venue.events[m].days[d].times[event.time] = event;
							}
						}
...
// multi venues
		...
		/*S.UpdateAvailability = function(categoryId) {
			let category = S.categories[categoryId],
					count = category.availability - category.tickets;
			...
			if (typeof category.remainders === 'undefined') {
				...
				for (let i in category.remainders) {
					let id = category.remainders[i];
					if (id != categoryId) {
						count -= S.categories[id].tickets;
					}
				}
				
				for (let i in category.remainders) {
					let id = category.remainders[i];
					S.categories[id].count = count;
				}
			}
		};*/
		S.UpdateAvailability = function(categoryId) {
