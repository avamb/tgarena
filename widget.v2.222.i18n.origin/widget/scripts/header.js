...
(function(){
	...
	function bilHeader(
		...
		S.openMenu = function(){
			...
			$.B24.menu_html.classList.add('lkmlkm');
		...
		S.SetEmail = function(){
			S.formError = false;
			if (typeof S.email === 'undefined') {
				S.formError = true;
				S.formMessage = S.translate.email.form.bind.required;
			} else if (S.email == S.config.oldEmail) {
				S.formError = true;
				S.formMessage = S.translate.email.form.bind.confirmed;
			} else {
				S.formMessage = S.translate.email.form.bind.send + ' ' + S.email;
				let request = R.GetRequest($.B24.params({command: 'BIND_EMAIL', email: S.email}, S.config, LS));
				if (request) {
					request.$promise.then(function(response){
						if (response.resultCode != 0) {
							S.formError = true;
							S.formMessage = response.description;
						
						} else {
							document.body.dispatchEvent(new Event('BIND_EMAIL', {
								bubbles: true,
							}));
		...
		S.ConfirmEmail = function(){
			S.formError = false;
			let request = R.GetRequest($.B24.params({command: 'CONFIRM_EMAIL', code: S.code}, S.config, LS));
			if (request) {
				request.$promise.then(function(response){
					if (response.resultCode != 0) {
						S.formError = true;
						...
						document.body.dispatchEvent(new Event('CONFIRM_EMAIL', {
							bubbles: true,
						}));
						
						S.config.email = S.email = response.email;
						growl.success(S.translate.email.form.confirm.success);
						S.confirm = false;
