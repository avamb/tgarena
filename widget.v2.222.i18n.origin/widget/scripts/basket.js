		...
		/*S.UpdateEntryList = function(cid, tid, qty){
			if (qty < 0) {
				...
				if (typeof S.categories[cid] !== 'undefined') {
					...
					if (S.categories[cid].count > S.categories[cid].availability) S.categories[cid].count = S.categories[cid].availability;
					
					if (typeof S.categories[cid].remainders !== 'undefined') {
						for (let i in S.categories[cid].remainders) {
							i = S.categories[cid].remainders[i];
							if (typeof S.categories[i] !== 'undefined') {
								S.categories[i].count = S.categories[cid].count;
							}
						}
					}
				}
