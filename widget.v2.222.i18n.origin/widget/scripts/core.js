	...
	$.B24 = {
		...
		Request: function($resource, $location){
			...
			//let params = $location.search(),
					requestURL = $.B24.i18n.requestURL;
					...
					if (request) {
						request.$promise.then(function(response){
							if (response.resultCode != 0) {
								...
								S.formError = false;
								if (S.config.email) {
									S.formMessage = S.translate.email.message.authorize + ' ' + S.config.email;
								} else {
									S.formMessage = S.translate.email.message.anonimus;
								}
								if (typeof S.email !== 'undefined') {
									if (S.email == S.config.email) {
										S.formMessage = S.translate.email.confirmed + ' ' + S.email;
									} else {
		...
		localization: function(config, params){
			...
			if (typeof params.success !== 'undefined' && params.success != 'appsuccess') {
				config.successUrl = params.success;
			} else {
				config.successUrl = $.B24.i18n.successUrl;
			}
			if (typeof params.fail !== 'undefined' && params.fail != 'appfail') {
				config.failUrl = params.fail;
			} else {
				config.failUrl = $.B24.i18n.failUrl;
			}
