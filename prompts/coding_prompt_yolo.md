...
### DEPLOYMENT CONTRACT (ONLY if app_spec requests Dokploy)

If `app_spec.txt` includes:
- `<deployment><target>dokploy</target>` AND `<mode>docker_compose</mode>`

Then these files MUST exist at the project root and MUST stay correct:
- `Dockerfile`
- `docker-compose.yml`
- `.env.example` (include `PORT=3000`)
- `deploy.dokploy.json` (must match actual runtime port & healthcheck)

**Port contract (CRITICAL):**
- The app MUST listen on `0.0.0.0:$PORT` (NOT localhost)
- `PORT` defaults to **3000**
- Do NOT hardcode a different port unless the spec says so; if you change it, update `deploy.dokploy.json` and `.env.example`
